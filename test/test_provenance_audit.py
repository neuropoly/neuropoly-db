...
Provenance is stored on manager.provenance after execute() returns.
"""
...
class TestProvenanceStructure:
    ...
    @pytest.mark.asyncio
    async def test_provenance_has_required_top_level_fields(
        ...
        with patch(
            "npdb.external.neurobagel.automation.NBAnnotationToolBrowserSession"
    ...
    @pytest.mark.asyncio
    async def test_provenance_mode_field_valid(
        self, synthetic_tsv: Path, output_dir: Path
    ):
        ...
        for mode in ["manual", "assist", "auto", "full-auto"]:
            ...
            with patch(
                ...
                await manager.execute(
                    participants_tsv_path=synthetic_tsv, output_dir=output_subdir
                )
                report = manager.provenance
            ...
            assert (
                report.mode == mode
            ), f"Mode mismatch: expected {mode}, got {report.mode}"

    @pytest.mark.asyncio
    async def test_provenance_timestamp_valid(
        self, synthetic_tsv: Path, output_dir: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_provenance_run_id_unique(
        self, synthetic_tsv: Path, output_dir: Path
    ):
        ...
        for i in range(2):
            ...
            with patch(
                ...
                await manager.execute(
                    participants_tsv_path=synthetic_tsv, output_dir=subdir
                )
                report = manager.provenance
...
class TestProvenanceCompleteness:
    ...
    @pytest.mark.asyncio
    async def test_per_column_tracking_present(
        self, synthetic_tsv: Path, output_dir: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_provenance_column_mapping_structure(
        self, synthetic_tsv: Path, output_dir: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_provenance_confidence_values_valid(
        self, synthetic_tsv: Path, output_dir: Path
    ):
...
class TestProvenanceWarnings:
    ...
    @pytest.mark.asyncio
    async def test_full_auto_mode_emits_warning(
        self, synthetic_tsv: Path, output_dir: Path
    ):
        ...
        assert (
            "full-auto" in warning_text
            or "experimental" in warning_text
            or "unstable" in warning_text
        )

    @pytest.mark.asyncio
    async def test_assist_mode_no_experimental_warning(
        self, synthetic_tsv: Path, output_dir: Path
    ):
...
class TestProvenanceSummary:
    ...
    @pytest.mark.asyncio
    async def test_mapping_source_counts_exist(
        self, synthetic_tsv: Path, output_dir: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_confidence_distribution_valid(
        self, synthetic_tsv: Path, output_dir: Path
    ):
